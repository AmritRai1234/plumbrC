...
# gRPC :50051 (streaming redaction)

echo "Starting PlumbrC HTTP Server on port 8081..."
/app/build/bin/plumbr-server --port 8081 --threads ${PLUMBR_THREADS:-4} &
PLUMBR_PID=$!

echo "Starting PlumbrC gRPC Server on port 50051..."
/app/build/bin/plumbr-grpc --port 50051 --threads ${PLUMBR_THREADS:-4} &
GRPC_PID=$!
...
echo "║  gRPC      :50051 streaming redaction    ║"
echo "║  Next.js   :3001  dashboard (internal)   ║"
echo "╚══════════════════════════════════════════╝"
echo ""

# Handle signals — graceful shutdown
trap "kill $NGINX_PID $PLUMBR_PID $GRPC_PID $NEXT_PID 2>/dev/null; exit 0" SIGTERM SIGINT

# Wait for any process to exit
wait -n $NGINX_PID $PLUMBR_PID $GRPC_PID $NEXT_PID
kill $NGINX_PID $PLUMBR_PID $GRPC_PID $NEXT_PID 2>/dev/null
